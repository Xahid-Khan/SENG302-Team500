...
public class ChangePasswordService {
    ...
    public ChangePasswordResponse changePassword(ChangePasswordRequest request) throws NoSuchAlgorithmException, InvalidKeySpecException {
        UserModel userDetails = repository.findById(request.getUserId());
        boolean changeSuccess = passwordService.verifyPassword(request.getCurrentPassword(), userDetails.getPasswordHash());
        String message;
        if (changeSuccess) {
            try {
                UserModel newUser =
