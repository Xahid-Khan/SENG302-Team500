...
public class GroupRepositoryController extends AuthenticatedController {
    ...
    /**
     * This method will be invoked when API receives a POST request to update a group. Invokes the
     * client service to update the group shortName, longName, repositoryID, and token
     *
     * @param groupId The group id to update
     * @param newGroup The contract for the group to update
     */
//    @PostMapping(value = "/groups/repository/{groupId}", produces = "application/json")
...
//        if (isTeacher(principal)) {
//            try {
//                UpdateGroupResponse response = groupsClientService.updateGroup(Integer.parseInt(groupId), newGroup);
//                if (response.getIsSuccess()) {
//                    return ResponseEntity.ok().build();
//                } else {
//                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Oh dear");
