...
class GroupFeedControllerTest {
  ...
  void setupBeforeEach() {
    Mockito.when(groupsClientService.getGroupById(any(int.class)))
        .thenReturn(GroupDetailsResponse.newBuilder()
            .setGroupId(1)
  ...
  void createNewPostWithValidFieldsAndExpectPass() throws Exception {
    Mockito.when(groupsClientService.isMemberOfTheGroup(any(int.class), any(int.class)))
        .thenReturn(true);
  ...
  /**
   * This test checks if the User how is not member of the group can post on that group feed page or
   * not, and it fails as expected.
   *
   * @throws Exception
   */
  @Test
  void createNewPostUserNotMemberOfGroupExpectFail() throws Exception {
    Mockito.when(groupsClientService.isMemberOfTheGroup(any(int.class), any(int.class)))
        .thenReturn(false);
    Mockito.when(postService.createPost(new PostContract(1, "This a test dummy post"), 1))
        .thenReturn(true);
    mockMvc.perform(post("/group_feed/new_post")
            .contentType(MediaType.APPLICATION_JSON)
            .content("""
  ...
  /**
   * This tests deletion of a post, the user who made the post is deleting the post, so it is
   * expected to pass.
   *
   * @throws Exception
   */
  ...
  /**
   * this tests deleteion of a post made by a different user, and it fails as expected as a user
   * cannot delete posts made by other users.
   *
   * @throws Exception
   */
  @Test
  ...
  /**
   * A post cannot be updated by any user other than the user who made the post, Event
   * Teachers/Admins cannot edit a post.
   *
   * @throws Exception
   */
  @Test
