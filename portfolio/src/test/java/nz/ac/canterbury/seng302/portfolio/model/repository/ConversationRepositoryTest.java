...
class ConversationRepositoryTest {
  ...
  private final Pageable request = PageRequest.of(0, 3, Sort.by(""));
  ...
  void testGetPaginatedPostsByUserIdsInAllConversations() {
    var result = conversationRepository
        .getPaginatedPostsByUserIdsIn(List.of(2), request)
        .getTotalElements();
  ...
  void testGetPaginatedPostsByUserIdsInTwoConversations() {
    var result = conversationRepository
        .getPaginatedPostsByUserIdsIn(List.of(3), request)
        .getTotalElements();
  ...
  void testEnsureMostRecentConversationIsDisplayedFirstWhenUpdated() {
    conversation3.addMessage(new MessageEntity("Hello world", 2));
    var result = conversationRepository
        .getPaginatedPostsByUserIdsIn(List.of(3), request);
